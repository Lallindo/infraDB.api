...
async def get_produtos(
    ...
    stmt = text("\
        SELECT p.*, m.descritivo_marca as marca_prod, c.descritivo_categoria as categoria_prod \
        FROM produtos p\
        LEFT JOIN marcas m ON m.id_marca = p.marca_prod\
        LEFT JOIN categorias c ON c.id_categoria = p.categoria_prod\
        ORDER BY p.id_prod ASC\
        LIMIT :limit\
        OFFSET :offset\
        ")
    produtos_data = db.execute(stmt, {"limit": limit, "offset": offset}).mappings().all()
    return {
        'produtos': produtos_data, 
        'paginacao': {'limit': limit, 'offset': offset, 'total': db.query(ProdutosDB).count()}} 
...
async def get_kits(
    ...
    return_val = em_kit.ProdutoComposicaoResponse(**produto.__dict__, faz_parte_de=db_resp)
